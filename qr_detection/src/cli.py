...
class QRDetectionCLI:
    ...
    def __init__(self):
        """Initialize the CLI."""
        self.qr_detection_root = Path(__file__).resolve().parent.parent  # Up to qr_detection/
        self.project_root = self.qr_detection_root.parent  # Up to project root
        self.runs_dir = self.qr_detection_root / "runs" / "detect"
        self.data_yaml = self.qr_detection_root / "data" / "data.yaml"  # Data now in qr_detection/
