...
```bash
# See all available models with training info
python -m src.qr_detection.cli --list

# Analyze a single image (interactive)
python -m src.qr_detection.cli --infer image.jpg

# Process entire folder
python -m src.qr_detection.cli --batch ./my_images/
```
...
```python
from src.qr_detection import quick_process
...
# Specify exact model
python -m src.qr_detection.cli --infer image.jpg --model ./src/qr_detection/runs/detect/qr_detector_v17/weights/best.pt

# Save results to disk
python -m src.qr_detection.cli --infer image.jpg --save-results
```

**Batch process directory:**
```bash
# Process all images in folder
python -m src.qr_detection.cli --batch ./images/

# With specific model and save results
python -m src.qr_detection.cli --batch ./images/ --model ./src/qr_detection/runs/detect/qr_detector_v17/weights/best.pt --save-results
```
...
```python
from src.qr_detection import QRImageProcessor

# Initialize processor with specific model
processor = QRImageProcessor(
    model_path="./src/qr_detection/runs/detect/qr_detector_v17/weights/best.pt"
)
...
Models are automatically discovered in: `src/qr_detection/runs/detect/`
...
# Python method
from src.qr_detection import QRDetectionCLI
cli = QRDetectionCLI()
...
```bash
# Using CLI
python -m src.qr_detection.cli --infer my_photo.jpg
...
# Latest model
python -m src.qr_detection.cli --infer image.jpg --model ./src/qr_detection/runs/detect/qr_detector_v17/weights/last.pt
```
...
**"No models found"**
- Check that trained models exist in: `src/qr_detection/runs/detect/`
...
- Use terminal command instead: `python -m src.qr_detection.cli --list`
...
2. **Test on single image**:
   ```bash
   python -m src.qr_detection.cli --infer test_image.jpg
   ```

3. **Batch process and save**:
   ```bash
   python -m src.qr_detection.cli --batch ./data/images/test/ --save-results
   ```
...
4. **Use in automation**:
   ...
   for image_file in Path("./src/qr_detection/data/images").glob("*.jpg"):
