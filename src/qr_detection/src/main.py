...
def print_options():
    ...
    options = {
        "1": {
            ...
            "command": "src.qr_detection.src.cli",
            "examples": [
                "List available models: python -m src.qr_detection.src.cli --list",
                "Detect QR in image: python -m src.qr_detection.src.cli --infer photo.jpg",
                "Batch process folder: python -m src.qr_detection.src.cli --batch ./images/",
            ],
        },
        "2": {
            "label": "GUI Interface",
            "description": "Graphical interface with clickable buttons",
            "command": "src.qr_detection.src.gui",
            "examples": [
                "Launch GUI: python -m src.qr_detection.src.gui",
...
def run_interactive_menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            subprocess.run(
                [sys.executable, "-m", "src.qr_detection.src.cli", "--list"],
                cwd=project_root,
            )
            print("\n📌 Next Steps:")
            print("   • Detect QR in image: python -m src.qr_detection.src.cli --infer photo.jpg")
            print("   • Batch process folder: python -m src.qr_detection.src.cli --batch ./images/")
            print("   • For more help: python -m src.qr_detection.src.cli --help\n")

        elif choice == "2":
            print("\n✅ Launching GUI Interface...\n")
            subprocess.run(
                [sys.executable, "-m", "src.qr_detection.src.gui"],
                cwd=project_root,
            )
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        subprocess.run([sys.executable, "-m", "src.qr_detection.src.cli"] + sys.argv[1:])
    else:
